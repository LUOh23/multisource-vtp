...
namespace geodesic {
	...
	public:
		...
		void propagate(const std::vector<unsigned>& sources);
...
//-------------------------- main entry (MODIFIED FOR MULTI-SOURCE) --------------------------
inline void GeodesicAlgorithmExact::propagate(const std::vector<unsigned>& sources)
{
	...
	for (unsigned src : sources) {
		...
		vertex_pointer source_vertex = &(this->mesh()->vertices()[m_source]);

		
		if (source_vertex->geodesic_distance() > 0.0) {
			source_vertex->geodesic_distance() = 0.0;
			source_vertex->state() = Vertex::INSIDE;
			
			create_pseudo_source_windows(source_vertex, false);
		}
	}
